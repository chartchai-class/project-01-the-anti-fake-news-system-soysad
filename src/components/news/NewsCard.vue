...
const status = computed(
  () => majorityLabel(item.fakeVotes, item.realVotes) as 'Fake' | 'Real' | 'Unclear',
)

const total = computed(() => Math.max(1, item.fakeVotes + item.realVotes))
const fakePct = computed(() => Math.round((item.fakeVotes / total.value) * 100))
...
<template>
  ...
  >
    ...
    <div class="p-5 flex flex-col grow">
      ...
      <p class="mt-1 text-sm text-zinc-600 line-clamp-2">{{ item.shortDetail }}</p>
      ...
      <div class="mt-auto pt-3 border-t border-zinc-100">
        ...
        <div
          ...
          :style="{ gridTemplateColumns: `${fakePct}% ${realPct}%` }"
          aria-label="Fake/Real ratio"
        >
          <div
          ...
          >
            ...
            >
              Fake {{ fakePct }}% ({{ item.fakeVotes }} / {{ total }})
            </div>
          </div>
          <div
            class="relative group cursor-help transition-colors"
          ...
          >
            ...
            >
              Real {{ realPct }}% ({{ item.realVotes }} / {{ total }})
            </div>
          </div>
        </div>
        <!-- ไม่มี status pill -->
